...
(() => {
  ...
  class DeckStage extends HTMLElement {
    ...
    _syncPrintPageRule() {
      ...
      // Signal that the deck is ready for headless capture (pdf.mjs waits on
      // this instead of an arbitrary setTimeout). We wait two animation frames
      // so the @page rule, slotted sections, fonts and any pending layout work
      // have all settled before flipping the flag.
      requestAnimationFrame(() => {
        requestAnimationFrame(() => {
          try { window.__deckReady = true; } catch {}
        });
      });
    }
